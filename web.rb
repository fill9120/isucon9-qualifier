  ...
  class Web < Sinatra::Base
    ...
    get '/new_items.json' do
      ...
      items = if item_id > 0 && created_at > 0
        # paging
        db.xquery("SELECT t1.*, t2.`account_name`, t2.`num_sell_items` FROM `items` AS t1 LEFT OUTER JOIN `users` AS t2 ON t1.`seller_id` = t2.`id` WHERE t1.`status` IN (?, ?) AND (t1.`created_at` < ?  OR (t1.`created_at` <= ? AND t1.`id` < ?)) ORDER BY t1.`created_at` DESC, t1.`id` DESC LIMIT #{ITEMS_PER_PAGE + 1}", ITEM_STATUS_ON_SALE, ITEM_STATUS_SOLD_OUT, Time.at(created_at), Time.at(created_at), item_id)
      else
        # 1st page
        db.xquery("SELECT t1.*, t2.`account_name`, t2.`num_sell_items` FROM `items` AS t1 LEFT OUTER JOIN `users` AS t2 ON t1.`seller_id` = t2.`id` WHERE t1.`status` IN (?, ?) ORDER BY t1.`created_at` DESC, t1.`id` DESC LIMIT #{ITEMS_PER_PAGE + 1}", ITEM_STATUS_ON_SALE, ITEM_STATUS_SOLD_OUT)
      end
    ...
    get '/new_items/:root_category_id.json' do
      ...
      category_ids = categories.find_all{|item| item[:parent_id] == root_category['id']}.map { |row| row[:id] }
    ...
    get '/users/transactions.json' do
      ...
      item_details = items.map do |item|
        ...
        if item['buyer_id'] != 0
          if item['buyer_account_name'].nil?
            db.query('ROLLBACK')
    ...
    get '/users/:user_id.json' do
      ...
      items = if item_id > 0 && created_at > 0
        # paging
        db.xquery("SELECT t1.* FROM `items` AS t1 LEFT OUTER JOIN `users` AS t2 ON t1.`seller_id` = t2.`id` WHERE t1.`seller_id` = ? AND t1.`status` IN (?, ?, ?) AND t1.`created_at` <= ? AND t1.`id` < ? ORDER BY t1.`created_at` DESC, t1.`id` DESC LIMIT #{ITEMS_PER_PAGE + 1}", user_simple['id'], ITEM_STATUS_ON_SALE, ITEM_STATUS_TRADING, ITEM_STATUS_SOLD_OUT, Time.at(created_at), item_id)
      else
        # 1st page
        db.xquery("SELECT t1.* FROM `items` AS t1 LEFT OUTER JOIN `users` AS t2 ON t1.`seller_id` = t2.`id` WHERE t1.`seller_id` = ? AND t1.`status` IN (?, ?, ?) ORDER BY t1.`created_at` DESC, t1.`id` DESC LIMIT #{ITEMS_PER_PAGE + 1}", user_simple['id'], ITEM_STATUS_ON_SALE, ITEM_STATUS_TRADING, ITEM_STATUS_SOLD_OUT)
      end
